...
for my $testfile ( sort @testfiles ) {
	...
	ok( ref $xml_epp eq 'XML::EPP',
	    'Check the templated in XML was parsed ok' );

	# check the round-tripping (same tests as earlier)
	XMLMappingTests::run_testset(
		$xml_epp->to_xml(),
		$yaml->{input_assertions},
	       );
	...
	if ( my $class = $yaml->{input_assertions}->{class} ) {
		...
		ok( $oClass eq $class,
		    "EPP: Correct object class ($oClass / $class)" );
	}
	...
	for my $srs_loop ( @{$yaml->{SRS}} ) {
		...
		my $srs_message = (
			$messages[0]->does("XML::SRS::Query")
				or $messages[0]->does("XML::SRS::Action")
			       );
		ok( $srs_message,
		    "Sane test: got SRS input when we expected it" );
		...
		if ( $srs_message ) {
			...
			# If the output from the proxy was SRS XML, we
			# need to pretend to hit the SRS
			if ( my $fake_response = $srs_loop->{fake_response} ) {
				my $message = XML::SRS::Response->parse(
					$srs_loop->{fake_response},
				       );
				my $rs_tx = SRS::EPP::SRSMessage->new(
					message => $message,
				       );

				# these 'parts' are
				# SRS::EPP::SRSResponse, which
				# notify() needs an array of
				@messages = $queue_item->notify(
					@{$rs_tx->parts()},
				       );
			}
		}
